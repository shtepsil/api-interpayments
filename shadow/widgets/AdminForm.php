...
class AdminForm extends Widget
{
    ...
    public function run()
    {
        ...
        if (!\Yii::$app->request->isAjax) {
            $content = $this->render('form/index', ArrayHelper::merge(
                [
                    'item' => $this->item,
            ...
            /*
             * Если main_container задан, и в нём не указан tag,
             * то по умолчанию будет tag => div
             */
            if (isset($this->params['main_container'])) {
                $tag = 'div';
                $options = [];
                if (isset($this->params['main_container']['tag'])) {
                    $tag = $this->params['main_container']['tag'];
                }
                if (isset($this->params['main_container']['options'])) {
                    $options = $this->params['main_container']['options'];
                }
...
//        d::pri($field_options);
        if (isset($config['type']) AND $config['type'] == 'hidden') {
            $field = $form->field($this->item, $key, $field_options)->hiddenInput()->label(false);
        } else {
...
JS
     ...
     * @return string
     */
    public function getRelationField($name, $attribute, $item = null, $config = [], $clone = false)
    {
        $id = 'new';
        $value = '';
