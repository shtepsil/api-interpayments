                    ...
                    <?php foreach ($attributes as $key => $value): ?>
                        <?php if (!is_array($value)): ?>
                            <? if ($model->hasAttribute($value)): ?>
                                <td class="name">
                                    <?= $context->getRelationField($name, $value, $item) ?>
                                </td>
                            <? endif ?>
                        <?php else: ?>
                            <td class="name">
                                <?= $context->getRelationField($name, $key, $item, $value) ?>
                            </td>
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <?php if ((isset($add) && $add != false) || !isset($add)): ?>
                ...
                <tr class="item">
                    <?php foreach ($attributes as $key=>$value): ?>
                        <?php if (!is_array($value)): ?>
                            <? if ($model->hasAttribute($value)): ?>
                                <td class="name">
                                    <?= $context->getRelationField($name, $value, null) ?>
                                </td>
                            <? endif; ?>
                        <?php else: ?>
                            <td class="name">
                                <?= $context->getRelationField($name, $key, null, $value) ?>
                            </td>
                        <?php endif; ?>
                    <?php endforeach; ?>
                ...
                <tr class="item hidden clone_<?= $name ?>">
                    <?php foreach ($attributes as $key=>$value): ?>
                        <?php if (!is_array($value)): ?>
                            <? if ($model->hasAttribute($value)): ?>
                                <td class="name">
                                    <?= $context->getRelationField($name, $value, null, [], true) ?>
                                </td>
                            <? endif ?>
                        <?php else: ?>
                            <td class="name">
                                <?= $context->getRelationField($name, $key, null, $value, true) ?>
                            </td>
                        <?php endif; ?>
                    <?php endforeach; ?>
                    <td class="actions text-center add-<?= $name ?>">
