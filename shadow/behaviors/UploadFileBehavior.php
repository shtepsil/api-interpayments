...
class UploadFileBehavior extends SBehavior
{
    ...
    /**
     * @var boolean $isFromBase64
     */
    public $isFromBase64 = false;
    ...
    protected function isBase64($base64String)
    {
        return preg_match('/^data:image\/(\w+);base64,/', $base64String, $type);
    }

    public function base64ToUploadedFile($base64String, $filename) {
        ...
        if (preg_match('/^data:image\/(\w+);base64,/', $base64String, $type)) {
            ...
            $tempFilePath = Yii::getAlias('@runtime/temp/managers/') . $filename . '.' . $extension;

            if (file_put_contents($tempFilePath, $fileData) === false) {
                throw new \Exception("Failed to write temporary file");
            }

            return new \yii\web\UploadedFile([
                'name' => $filename . '.' . $extension,
                'tempName' => $tempFilePath,
                'type' => 'image/' . $type,
                'size' => filesize($tempFilePath),
                'tempResource' => fopen($tempFilePath, 'r'),
                'error' => UPLOAD_ERR_OK,
            ]);
    ...
    public function addFiles($attribute)
    {
        ...
        if ($this->isFromBase64 && $this->isBase64($this->owner->{$attribute})) {
            $this->files[$attribute] = $this->base64ToUploadedFile($this->owner->{$attribute}, $attribute . '_'
                . (!empty($this->owner->id) ? $this->owner->id : uniqid()));
    ...
    public function beforeSave()
    {
        ...
        foreach ($this->attributes as $attribute) {
            ...
            if (isset($this->files[$attribute]) && $this->files[$attribute] instanceof UploadedFile) {
