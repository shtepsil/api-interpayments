$(function () {
    if (typeof $.fn.select2 != 'undefined') {
        $.fn.select2.defaults.set('theme', 'shadow');
    }
});
...
$('a[data-ajax]').on('click', function (e) {
    e.preventDefault();
    changeOrderStatus.apply(this);
});
$('.table-responsive table tbody').on('click', '.order-action', function () {
    changeOrderStatus.apply(this);
});
...
function changeOrderStatus() {
    ...
    if (typeof url == 'undefined') {
        ...
        typeRequest = $(button).attr('data-ajax');
        action = $(button).attr('data-action');
        reload = false;
        if ($(button).hasClass('disabled')) {
            return;
        } else {
            localStorage.setItem('blockDataTable', '1');
        }
    }
    ...
    $.ajax({
        ...
        .done(function (data) {
            ...
            if (reload) {
                ...
                if (action) {
                    if (action == 'lock') {
                        $(button).hide(10, function () {
                            $(button)
                                .next()
                                .fadeIn(100, function () {
                                    localStorage.setItem('blockDataTable', '0');
                                });
                        });
                    } else {
                        $(button).hide(10);

                        // Тут показывал кнопку "Принять", после нажатия на "Отмена"
                        // $(button).hide(10, function () {
                        //     $(button)
                        //         .prev()
                        //         .fadeIn(100, function () {
